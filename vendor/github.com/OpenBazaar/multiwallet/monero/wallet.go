...
func multisigNotImplementedError(methodName string) error {
	err := errors.New("Method '" + methodName + "': Multisig which is needed if vendor is currently offline is not implemented for Monero")
	return err
}
...
func (w *Wallet) CurrentAddress(purpose wi.KeyPurpose) btc.Address {
	...
	if purpose == wi.DIRECT_PAYMENT {
		// @@@ Little hack: Get the main address for offline direct payments to it
		return NewMoneroAddress(w.mainAddress)
	}
	w.lock()
	defer w.unlock()
...
// @@@ Experimental method for offline direct payments: Get an address to
// put into 'ID_Pubkeys.Monero'
// If this approach gets confirmed maybe a wallet config setting will be needed
// for being able to influence the returned address.
func (w *Wallet) DirectPaymentAddress() string {
	return w.mainAddress
}
...
func transferToTxn(transfer *walletrpc.Transfer, sign int64, status wi.StatusCode, blockchainHeight uint64) wi.Txn {
	return wi.Txn{Txid: transfer.TxID, Value: int64(transfer.Amount) * sign, Height: int32(transfer.Height),
		...
		Confirmations: int64(blockchainHeight - transfer.Height), Status: status,
	}
}
...
func (w *Wallet) transactionsUnlocked() ([]wi.Txn, error) {
	...
	req := walletrpc.RequestGetTransfers{In: true, Out: true, Pending: true}
	...
	for _, transfer := range resp.In {
		transaction := transferToTxn(transfer, 1, wi.StatusConfirmed, heightResp.Height)
	...
	for _, transfer := range resp.Pending {
		transaction := transferToTxn(transfer, -1, wi.StatusPending, heightResp.Height)
		transactions = append(transactions, transaction)
	}
	...
	w.logInfo("%d in txs, %d out txs, %d pending txs", len(resp.In), len(resp.Out), len(resp.Pending))
...
func (w *Wallet) GetTransaction(txid chainhash.Hash) (wi.Txn, error) {
	...
	var sign int64 = 1
	var status wi.StatusCode = wi.StatusConfirmed
	if transferResp.Transfer.Type == "out" {
		sign = -1
	} else if transferResp.Transfer.Type == "pending" {
		sign = -1
		status = wi.StatusPending
	}
	w.logInfo("Height %d", heightResp.Height)
	return transferToTxn(&transferResp.Transfer, sign, status, heightResp.Height), nil
}
...
func (w *Wallet) Spend(amount int64, addr btc.Address, feeLevel wi.FeeLevel, referenceID string, spendAll bool) (*chainhash.Hash, error) {
	// @@@@@@@@ Implement 'spendAll'
...
func (w *Wallet) AddWatchedAddresses(addrs ...btc.Address) error {
	...
	if addrs != nil {
		w.logInfo("Add watched address %s", addrs[0].String())
	}
	return nil
}
...
func transferToCallback(transfer *walletrpc.Transfer, address string, sign int64) wi.TransactionCallback {
	output := wi.TransactionOutput{Address: NewMoneroAddress(address),
		Value: int64(transfer.Amount)}
	return wi.TransactionCallback{Txid: transfer.TxID, Outputs: []wi.TransactionOutput{output},
		Height: int32(transfer.Height), Timestamp: time.Unix(int64(transfer.Timestamp), 0),
		Value: int64(transfer.Amount) * sign, WatchOnly: false, BlockTime: time.Unix(int64(transfer.Timestamp), 0)}
}
...
func (w *Wallet) listenerThread() {
	for {
		if w.client != nil {
			...
			if (len(resp.In) + len(resp.Out) + len(resp.Pending)) > 0 {
				w.logInfo("Check for new txs gave %d in, %d out, %d pending", len(resp.In), len(resp.Out), len(resp.Pending))
			} else {
				w.logDebug("Check for new txs gave %d in, %d out, %d pending", len(resp.In), len(resp.Out), len(resp.Pending))
			}
			var maxTxHeight uint64
			for _, transfer := range resp.In {
				// For an "In" transfer the receiving (sub)address is in 'transfer.Address' and not
				// in a 'destination' entry
				info := transferToCallback(transfer, transfer.Address, 1)
			...
			for _, transfer := range resp.Out {
				...
				info := transferToCallback(transfer, address, -1)
			...
			for _, transfer := range resp.Pending {
				...
				info := transferToCallback(transfer, address, -1)
				w.logDebug("Submit to listeners pending tx %s", transfer.TxID)
				for _, callback := range w.listeners {
					callback(info)
				}
				if transfer.Height > maxTxHeight {
...
// Implement the "WalletMustManuallyAssociateTransactionToOrder" interface for offline direct
// payments, to react to ORDER_PAYMENT messages
// Do not simply use "WalletService.InvokeTransactionListeners" like some other wallets because it's
// somewhat complicated to construct a WalletService ...
func (w *Wallet) AssociateTransactionWithOrder(txnCB wallet.TransactionCallback) {
